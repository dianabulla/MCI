...
$volverUrl = $returnUrl ?: (PUBLIC_URL . '?url=discipular/ministerios');
$metas = $metas ?? [];
...
<div class="form-container">
    ...
    <div class="alert alert-danger" style="margin-bottom:10px;">
        <?= htmlspecialchars($lpMsg !== '' ? $lpMsg : 'No se pudo guardar la configuración de líderes principales.') ?>
    </div>
    <?php endif; ?>
    ...
    <form method="POST">
        ...
        <div class="form-group">
            <label>Líderes principales del ministerio</label>
            <div class="lideres-principales-grid">
                <select name="id_lider_principal_1" class="form-control">
                    <option value="">Seleccionar líder principal 1</option>
                    <?php foreach ($candidatosLideresPrincipales as $cand): ?>
                        <?php
                            $idCand = (int)($cand['id_persona'] ?? 0);
                            $nomCand = trim((string)($cand['nombre'] ?? ''));
                            $rolCand = trim((string)($cand['rol'] ?? ''));
                            $minCand = trim((string)($cand['nombre_ministerio'] ?? ''));
                            if ($idCand <= 0) { continue; }
                        ?>
                        <option value="<?= $idCand ?>" <?= $idCand === $idLiderPrincipal1 ? 'selected' : '' ?>>
                            <?= htmlspecialchars($nomCand !== '' ? $nomCand : ('Persona ' . $idCand)) ?>
                            <?= $rolCand !== '' ? ' - ' . htmlspecialchars($rolCand) : '' ?>
                            <?= $minCand !== '' ? ' - ' . htmlspecialchars($minCand) : '' ?>
                        </option>
                    <?php endforeach; ?>
                </select>

                <select name="id_lider_principal_2" class="form-control">
                    <option value="">Seleccionar líder principal 2</option>
                    <?php foreach ($candidatosLideresPrincipales as $cand): ?>
                        <?php
                            $idCand = (int)($cand['id_persona'] ?? 0);
                            $nomCand = trim((string)($cand['nombre'] ?? ''));
