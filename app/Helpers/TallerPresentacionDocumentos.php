...
/**
 * Documentos adjuntos en formulario público Presentación de niños (talleres).
 */
class TallerPresentacionDocumentos {
    public const EXTENSIONES = ['pdf', 'jpg', 'jpeg', 'png', 'webp', 'doc', 'docx'];
    public const TAMANO_MAX = 8 * 1024 * 1024;
    public const INPUT_NAME = 'documentos_presentacion_ninos';
     ...
     */
    public function adjuntarDesdeUpload(int $idRespuesta, $filesInput, array $existentes = []): array {
        ...
        $dir = self::directorioRespuesta($idRespuesta);
        ...
        foreach ($archivos as $archivo) {
            ...
            $lista[] = [
                ...
                'archivo' => $nombreFinal,
                'url' => self::construirUrlArchivo($idRespuesta, $nombreFinal, ''),
     ...
     */
    public static function decodificar($raw, int $idRespuesta = 0): array {
        if (is_array($raw)) {
            $decoded = $raw;
        } else {
        ...
        foreach ($decoded as $item) {
            ...
            $out[] = [
                ...
                'archivo' => $archivo,
                'url' => self::construirUrlArchivo($idRespuesta, $archivo, trim((string)($item['url'] ?? ''))),
    ...
    /**
     * Documentos guardados en JSON y/o en disco para una respuesta.
     *
     * @param mixed $raw
     * @return array<int, array{nombre:string,archivo:string,url:string,fecha:string}>
     */
    public static function obtenerParaRespuesta(int $idRespuesta, $raw): array {
        $idRespuesta = (int)$idRespuesta;
        if ($idRespuesta <= 0) {
            return [];
        }
        ...
        $porArchivo = [];
        foreach (self::decodificar($raw, $idRespuesta) as $doc) {
            $clave = (string)($doc['archivo'] ?? '');
            if ($clave !== '') {
                $porArchivo[$clave] = $doc;
            }
        }
        foreach (self::listarDesdeDirectorio($idRespuesta) as $doc) {
            $clave = (string)($doc['archivo'] ?? '');
            if ($clave === '') {
                continue;
            }
     ...
     * @return array<int, array{nombre:string,archivo:string,url:string,fecha:string}>
     */
    public static function listarDesdeDirectorio(int $idRespuesta): array {
        $idRespuesta = (int)$idRespuesta;
        if ($idRespuesta <= 0) {
            return [];
        }
        ...
        foreach ($archivos as $nombreArchivo) {
            ...
            $out[] = [
                ...
                'url' => self::construirUrlArchivo($idRespuesta, $nombreArchivo, ''),
    ...
    public static function construirUrlArchivo(int $idRespuesta, string $archivo, string $urlExistente = ''): string {
        ...
        $idRespuesta = (int)$idRespuesta;
        if ($archivo === '' || $idRespuesta <= 0) {
            return '';
        }
        $basePublic = rtrim((string)(defined('PUBLIC_URL') ? PUBLIC_URL : ''), '/');
        return $basePublic . '/uploads/talleres_presentacion/' . $idRespuesta . '/' . rawurlencode($archivo);
    }
    ...
    public static function directorioRespuesta(int $idRespuesta): string {
        $root = defined('ROOT') ? ROOT : dirname(__DIR__, 2);
        return $root . '/public/uploads/talleres_presentacion/' . (int)$idRespuesta;
    }
